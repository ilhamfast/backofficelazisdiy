<div x-cloak x-show="isOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
    ...
    <div
        ...
        <div class="flex items-center justify-between">
            <h3 class="text-lg text-gray-900 mb-3 font-bold" id="modal-title">Edit
                Kabar Terbaru
            </h3>
        ...
        <!-- Formulir Campaign -->
        <form id="kabarterbaru-form-{{ $news['id'] }}"
            action="{{ route('news.update', $news['id']) }}" method="POST"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="latest_news_date" class="block text-left text-sm font-medium text-gray-700 mb-2">Tanggal
                        :</label>
                    <input type="date" name="latest_news_date" id="latest_news_date" value="{{ $news['latest_news_date'] }}"
                ...
                <div>
                    <label for="image_{{ $news['id'] }}" class="block text-sm font-medium text-gray-700 mb-2">
                        Image
                        <span class="italic text-xs">(Hanya file JPEG, JPG, atau PNG, max 2MB)</span>
                    </label>
                    <input type="file" name="image" id="image_{{ $news['id'] }}"
                        data-id="{{ $news['id'] }}" value="{{ $news['image'] }}"
                        class="mt-1 block w-full py-1.5 px-2 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border"
                        accept="image/*">

                    <!-- Preview Thumbnail -->
                    <div id="previewContainer_{{ $news['id'] }}"
                        class="relative mt-3 w-40 h-auto rounded-lg border border-gray-300 {{ empty($news['image']) ? 'hidden' : '' }}">
                        <img id="thumbnailPreview_{{ $news['id'] }}" src="{{ $news['image'] ?? '' }}"
                            alt="Preview Thumbnail" class="w-full h-auto">
                        <button id="resetButton_{{ $news['id'] }}" type="button"
                            class="absolute top-1 right-1 text-white bg-red-500 rounded-full w-6 h-6 flex items-center justify-center">
                            &times;
                        </button>
                    </div>
                    <p id="imageerror_{{ $news['id'] }}" class="hidden text-sm text-left text-red-500 mt-1">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3"
                        ...
                        placeholder="Masukkan deskripsi" required>{{ $news['description'] }}</textarea>
                </div>
