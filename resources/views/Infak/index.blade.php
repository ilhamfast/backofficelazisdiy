...
<body class="h-screen bg-gray-100 overflow-x-hidden">
                ...
                <div class="bg-white col-span-3 rounded-md mt-2 mx-2 shadow-xl">
                    <div class="mx-5">
                        <div class="flex justify-between items-center mb-6">
                            <div class="mt-2">
                                <h1 class="text-2xl font-semibold text-gray-900">Infak</h1>
                            </div>
                            ...
                            <div x-data="{ isOpen: false }" class="relative mt-10">
                                ...
                                <button type="button" @click="isOpen = true"
                                    class="bg-green-600 p-2 rounded-md shadow-md text-white">Buat Infak</button>

                                <div>
                                    @include('infak.create')
                                </div>
                            </div>
                                        ...
                                        @foreach ($infaks as $index => $infak)
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    {{ $infak['category_name'] }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    Rp {{ number_format($infak['amount'], 0, ',', '.') }}
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    Rp {{ number_format($infak['distribution'], 0, ',', '.') }}
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    <div class="flex items-center gap-2">
                                                        <div x-data="{ isOpen: false }">
                                                            <button class="text-yellow-800" type="button"
                                                                value="{{ $infak['id'] }}" @click="isOpen = true">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('infakForm');
            ...
            function validateInput(value) {
                if (!value.trim()) {
                    errorMessage.textContent = 'Nama kategori infak tidak boleh kosong.';
            ...
            input.addEventListener('input', debounce(function() {
                const infakCategory = this.value;

                if (!validateInput(infakCategory)) {
                    return;
                }

                fetch('{{ route('infak.check') }}', {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            category_name: infakCategory.trim()
                        })
                    })
                    .then(response => {
                        if (!response.ok) {
            ...
            form.addEventListener('submit', function(e) {
                const infakCategory = input.value;
