...
<div x-cloak x-show="isOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95"
    ...
    <div
        ...
        <div class="flex items-center justify-between">
            <h3 class="text-lg font-medium text-gray-900 mb-3" id="modal-title">Edit Zakat</h3>
        ...
        <!-- Formulir -->
        <form id="editzakatForm" action="{{ route('zakat.update', $zakat['id']) }}" method="POST"
            ...
            <div>
                <!-- Nama Zakat -->
                <div>
                    <label for="category_name" class="block text-sm font-medium text-gray-700 mb-2">Nama Zakat</label>
                    <input type="text" name="category_name" id="editcategory_name"
                        ...
                        placeholder="Masukkan nama zakat..." value="{{ $zakat['category_name'] }}" required>
                </div>
                ...
                <div class="mt-4">
                    ...
                    <div>
                        <label id="labelnamefile"
                            class="block text-sm text-gray-500 italic">{{ basename($zakat['thumbnail'] ?? 'Belum ada file') }}</label>
                    ...
                    <div id="previewContaineredit"
                        class="relative mt-3 w-40 h-auto rounded-lg border border-gray-300 {{ empty($zakat['thumbnail']) ? 'hidden' : '' }}">
                        <img id="thumbnailPreviewedit" src="{{ $zakat['thumbnail'] ?? '' }}" alt="Preview Thumbnail"
                            class="w-full h-auto">
...
<script>
    ...
    thumbnailInput.addEventListener('change', function(e) {
        ...
        if (validateFile(file)) {
            ...
            thumbnailPreview.src = "{{ $zakat['thumbnail'] ?? '' }}"; // Reset ke gambar lama
    ...
    resetButton.addEventListener('click', function() {
        ...
        thumbnailPreview.src = "{{ $zakat['thumbnail'] ?? '' }}";
        previewContainer.classList.remove('hidden');
